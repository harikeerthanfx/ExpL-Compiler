...
/* One formal parameter in a function signature.  This belongs to a function's
 * global-symbol-table entry; local bindings are deliberately a later task. */
struct Paramstruct {
    char *name;
    int type;
...
/* Temporary parser representation of one item in a global declaration. */
struct GDeclItem {
    char *name;
    int size;
    int rows;
...
struct Gsymbol { //for TASK1 (This represents one entry in the symbol table.)
    ...
    int binding;
    struct Paramstruct *paramlist; /* non-NULL only for a function */
    int flabel;                    /* F0, F1, ...; -1 for a variable */
    int defined;                   /* whether its definition was seen */
    struct TupleType *tupleType;   /* non-NULL for tuple variables */
    struct Gsymbol *next;
};
...
typedef struct tnode { 
    ...
    struct Lsymbol *Lentry;   // Task 2: entry when a local shadows a global
    struct ArgList *arglist;  // Task 2: actual arguments of a function call
    struct TupleType *tupleType;
    struct Field *field;
...
/* A completed semantic unit retained until Task 3 emits its code.  entry is
 * NULL for main, which has no GST declaration in this grammar. */
struct FunctionAST {
    char *name;
    ...
    tnode *returnExpr;
    int localCount;
    struct Lsymbol *localSymbols; /* retained while AST nodes reference it */
...
/* Task 2 local-scope and function-definition semantic analysis. */
