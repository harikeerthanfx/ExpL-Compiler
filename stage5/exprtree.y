...
%type <fieldlist> FieldList Field
%type <tuplevarlist> TupleVarList TupleVar
...
Decl : Type GidList SEMICOLON {
    ...
    | TUPLE ID '(' FieldList ')' TupleVarList SEMICOLON {
        InstallTupleType($2, $4);
        InstallTupleVariables($2, $6, 0);
    }
    ;
...
Field : Type ID { $$ = makeField($2, $1); }
      | Type MUL ID { $$ = makeField($3, $1 == TYPE_INT ? TYPE_INT_PTR : TYPE_STR_PTR); }
      ;

TupleVarList : TupleVarList ',' TupleVar { $$ = appendTupleVar($1, $3); }
             | TupleVar { $$ = $1; }
             ;

TupleVar : ID { $$ = makeTupleVar($1, 0); }
         | MUL ID { $$ = makeTupleVar($2, 1); }
         ;
...
Gid : ID {
    ...
    | MUL ID '(' ParamListOpt ')' {
        $$ = calloc(1, sizeof(*$$)); $$->name = $2; $$->isFunction = 1;
        $$->isPointer = 1; $$->paramlist = $4;
    }
    | ID '(' ParamListOpt ')' {
...
Param : Type ID {
    ...
    | Type MUL ID {
        $$ = calloc(1, sizeof(*$$));
        $$->type = $1 == TYPE_INT ? TYPE_INT_PTR : TYPE_STR_PTR;
        $$->name = $3;
    }
    ;
...
/* Only function return types use this production.  Global/local variable
 * declarations continue using Gid's existing '*' syntax. */
ReturnType : Type { $$ = $1; }
           | Type MUL { $$ = $1 == TYPE_INT ? TYPE_INT_PTR : TYPE_STR_PTR; }
           ;
...
Fdef : ReturnType ID '(' ParamListOpt ')' {
            BeginFunctionDefinition($1, $2, $4);
       } '{' LocalDeclBlockOpt FunctionBody '}'
     ;
...
LocalDeclBlockOpt : LocalDeclBlock
                  | /* empty */
                  ;
